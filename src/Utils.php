 ...
 */
  public static function safeEncode($value,$safe=false)
  {
    if(is_string($value))
    {
      ...
      return self::safeEncode(get_object_vars($value),$safe);
    } elseif(is_array($value)) {
      $es=array();
      if(($n=count($value))>0 && array_keys($value)!==range(0,$n-1))
      {
        foreach($value as $k=>$v) {
          $es[]="'".self::quote($k)."':".self::safeEncode($v,$safe);
        }
        return '{'.implode(',',$es).'}';
      }
      else
